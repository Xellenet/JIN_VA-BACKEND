...
export class CreateDirectMessages1782300000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(
      `ALTER TABLE "direct_messages" ADD CONSTRAINT "FK_dm_sender" FOREIGN KEY ("sender_id") REFERENCES "users"("id") ON DELETE CASCADE`,
    );
    await queryRunner.query(
      `ALTER TABLE "direct_messages" ADD CONSTRAINT "FK_dm_receiver" FOREIGN KEY ("receiver_id") REFERENCES "users"("id") ON DELETE CASCADE`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "direct_messages" DROP CONSTRAINT "FK_dm_receiver"`,
    );
    await queryRunner.query(
      `ALTER TABLE "direct_messages" DROP CONSTRAINT "FK_dm_sender"`,
    );
